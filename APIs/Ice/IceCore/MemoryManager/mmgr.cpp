...
static	const	unsigned int	paddingSize            = 1024;
#else
static	const	unsigned int	hashBits               = 12;
...
static		int		currentAllocationCount = 0;
static		int		breakOnAllocationCount = -1;
...
{
	fprintf(fp, "   Addr       Size       Addr       Size                        BreakOn BreakOn              \r\n");
	...
	for (unsigned int i = 0; i < hashSize; i++)
	{
		...
		while(ptr)
		{
			fprintf(fp, "%10p 0x%08X %10p 0x%08X 0x%08X %-8s    %c       %c    %s\r\n",
				ptr->reportedAddress, ptr->reportedSize,
				ptr->actualAddress, ptr->actualSize,
				m_calcUnused(ptr),
...
void	*m_allocator(const char *sourceFile, const unsigned int sourceLine, const char *sourceFunc, const unsigned int allocationType, const size_t reportedSize)
{
	try
	{
		...
		if (alwaysLogAll) log("%-40s %8s            : %s", ownerString(sourceFile, sourceLine, sourceFunc), allocationTypes[allocationType], memorySizeString(reportedSize));
...
void	*m_reallocator(const char *sourceFile, const unsigned int sourceLine, const char *sourceFunc, const unsigned int reallocationType, const size_t reportedSize, void *reportedAddress)
{
	try
	{
		...
		if (alwaysLogAll) log("%-40s %8s(%010p): %s", ownerString(sourceFile, sourceLine, sourceFunc), allocationTypes[reallocationType], reportedAddress, memorySizeString(reportedSize));
